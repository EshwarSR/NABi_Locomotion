...
for n = 1:1:iter
    ...
    elseif( n >= iter/8 && n < 3*iter/8 )
        
        th1d(n,1) = -(pi/12) + ((th1desired(1,1) + pi/12)/(iter*tstep/4))*(n-iter/8)*tstep;    
        th2d(n,1) = (pi/12) + ((th2desired(1,1) - pi/12)/(iter*tstep/4))*(n-iter/8)*tstep;      
        th3d(n,1) = -(pi/12) + ((th3desired(1,1) + pi/12)/(iter*tstep/4))*(n-iter/8)*tstep;     
        th4d(n,1) = (pi/12) + ((th4desired(1,1) - pi/12)/(iter*tstep/4))*(n-iter/8)*tstep; 
...
s = [ th2(1,1); th4(1,1); yL(1,1); zL(1,1); phi(1,1); dth2(1,1); dth4(1,1); dyL(1,1); dzL(1,1); dphi(1,1);...
      th1(1,1); th3(1,1); yR(1,1); zR(1,1); dth1(1,1); dth3(1,1); dyR(1,1); dzR(1,1); yCOM(1,1); zCOM(1,1); dyCOM(1,1); dzCOM(1,1) ] ;
...
for n = 1:1:iter
    ...
    dth1(n,1) = s(15); dth3(n,1) = s(16); dyR(n,1) = s(17); dzR(n,1) = s(18); yCOM(n,1) = s(19); zCOM(n,1) = s(20); dyCOM(n,1) = s(21); dzCOM(n,1) = s(22);
    ...
    ddth2s = ddyEL(n,1) - ddyL(n,1) ...
            ...
            + lF*cos(phi(n,1) + th2(n,1))*(dphi(n,1) + dth2(n,1))^2 ;
    ...
    ddth4s = ddzEL(n,1) - ddzL(n,1) ...
            ...
            + lF*sin(phi(n,1) + th2(n,1))*(dphi(n,1) + dth2(n,1))^2 ;
    ...
    ddth1s = ddyER(n,1) - ddyR(n,1) ...
            ...
            - lF*cos(phi(n,1) + th1(n,1))*(dphi(n,1) + dth1(n,1))^2 ;
    ...
    ddth3s = ddzER(n,1) - ddzR(n,1) ...
            ...
            - lF*sin(phi(n,1) + th1(n,1))*(dphi(n,1) + dth1(n,1))^2 ;
   ...
   %% NORMAL REACTION FRICTION FORCE BALANCE
    ...
    if( zL(n,1) < 0 )
        if (dzL(n,1) < 0)
            NL = (M + mT + mF + m)*g + m*(-dzL(n,1) - dzL(n,1))/tstep;
            dzL(n,1) = -0.1*dzL(n,1);
            zL(n,1) = 0 ;
        else
            NL = (M + mT + mF + m)*g ;
        end
         FkL = - m*ddyL(n,1) + m*(-dyL(n,1) - dyL(n,1))/tstep ;
         dyL(n,1) = 0;
    else       
        FkL = 0 ;
        NL = 0 ;
    ...
    if( zR(n,1) < 0 )  
        if (dzR(n,1) < 0)
            NR = (M + mT + mF + m)*g + m*(-dzR(n,1) - dzR(n,1))/tstep;
            dzR(n,1) = -0.1*dzR(n,1);
            zR(n,1) = 0;
        else 
            NR = (M + mT + mF + m)*g ;
        end
         FkR = - m*ddyR(n,1) + m*(-dyR(n,1) - dyR(n,1))/tstep ;
         dyR(n,1) = 0;
    else       
        FkR = 0 ;
        NR = 0 ;
...
%% CENTER OF MASS DYNAMICS
    ...
    dyEL(n,1) = dyL(n,1) ...
            - lT*sin(phi(n,1) + th2(n,1) + th4(n,1))*(dphi(n,1) + dth2(n,1) + dth4(n,1)) ...
            - lF*sin(phi(n,1) + th2(n,1))*(dphi(n,1) + dth2(n,1)) ;
    ...
    dzEL(n,1) = dzL(n,1) ...
            + lT*cos(phi(n,1) + th2(n,1) + th4(n,1))*(dphi(n,1) + dth2(n,1) + dth4(n,1))  ...
            + lF*cos(phi(n,1) + th2(n,1))*(dphi(n,1) + dth2(n,1)) ;
    ...
    dyER(n,1) = dyR(n,1) ...
            + lT*sin(phi(n,1) + th1(n,1) + th3(n,1))*(dphi(n,1) + dth1(n,1) + dth3(n,1))  ...
            + lF*sin(phi(n,1) + th1(n,1))*(dphi(n,1) + dth1(n,1)) ;
    ...
    dzER(n,1) = dzR(n,1) ...
            - lT*cos(phi(n,1) + th1(n,1) + th3(n,1))*(dphi(n,1) + dth1(n,1) + dth3(n,1))  ...
            - lF*cos(phi(n,1) + th1(n,1))*(dphi(n,1) + dth1(n,1)) ;

    ddyCOM(n,1) = 0.5*ddyEL(n,1) + 0.5*ddyER(n,1) ;
    ddzCOM(n,1) = 0.5*ddzEL(n,1) + 0.5*ddzER(n,1) ;
    dyCOM(n,1) = 0.5*dyEL(n,1) + 0.5*dyER(n,1) ;
    dzCOM(n,1) = 0.5*dzEL(n,1) + 0.5*dzER(n,1) ;
    ddphi(n,1) = (ddyEL(n,1)*sin(phi(n,1)) - ddzEL(n,1)*cos(phi(n,1))) - (ddyER(n,1)*sin(phi(n,1)) - ddzER(n,1)*cos(phi(n,1))) ; 
    dphi(n,1) = (dyEL(n,1)*sin(phi(n,1)) - dzEL(n,1)*cos(phi(n,1))) - (dyER(n,1)*sin(phi(n,1)) - dzER(n,1)*cos(phi(n,1))) ; 
    ddphi(n,1) = 0;
    dphi(n,1) = 0;
    ...
    dyEL(n,1) = dyCOM(n,1) + 0.5*d*dphi(n,1)*sin(phi(n,1)) ;
    dzEL(n,1) = dzCOM(n,1) - 0.5*d*dphi(n,1)*cos(phi(n,1)) ;
    dyER(n,1) = dyCOM(n,1) - 0.5*d*dphi(n,1)*sin(phi(n,1)) ;
    dzER(n,1) = dzCOM(n,1) + 0.5*d*dphi(n,1)*cos(phi(n,1)) ;

%% SYSTEM STATE SPACE EULER RUNGE KUTTA SOLVER

    s0 = [ th2(n,1); th4(n,1); yL(n,1); zL(n,1); phi(n,1); dth2(n,1); dth4(n,1); dyL(n,1); dzL(n,1); dphi(n,1); th1(n,1); th3(n,1); yR(n,1); zR(n,1); dth1(n,1); dth3(n,1); dyR(n,1); dzR(n,1); yCOM(n,1); zCOM(n,1); dyCOM(n,1); dzCOM(n,1) ] ;
    sdot = [ dth2(n,1); dth4(n,1); dyL(n,1); dzL(n,1); dphi(n,1); ddth2(n,1); ddth4(n,1); ddyL(n,1); ddzL(n,1); ddphi(n,1); dth1(n,1); dth3(n,1); dyR(n,1); dzR(n,1); ddth1(n,1); ddth3(n,1); ddyR(n,1); ddzR(n,1); dyCOM(n,1); dzCOM(n,1); ddyCOM(n,1); ddzCOM(n,1) ] ; 
