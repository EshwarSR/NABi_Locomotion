        ...
        for i in range(num_rollouts):
            ...
            if evaluate:
                ...
                reward, r_steps = self.rollout(
                    shift=0., rollout_length=self.env.spec.max_episode_steps)
...
class ARSLearner(object):
    ...
    def __init__(self, env_name='HalfCheetah-v1',
        ...
        log_filename = "seed_"+str(seed)+".txt"
        params_filename = "params_"+str(seed)+".json"
        logz.configure_output_dir(logdir, log_filename)
        logz.save_params(params, params_filename)
    ...
    def train_step(self):
        ...
        self.w_policy -= self.optimizer._compute_step(
            g_hat).reshape(self.w_policy.shape)
        return
        ...
        for i in range(num_iter):
            ...
            if ((i + 1) % 10 == 0):
                ...
                rewards = self.aggregate_rollouts(
                    num_rollouts=100, evaluate=True)
                w = ray.get(self.workers[0].get_weights_plus_stats.remote())
                np.savez(self.logdir + "/lin_policy_plus_seed" +
                         str(self.seed), w)
            ...
            setting_filters_ids = [worker.sync_filter.remote(
                filter_id) for worker in self.workers]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_name', type=str, default='NabiSlope-v0')
    ...
    parser.add_argument('--n_workers', '-e', type=int, default=11)
    parser.add_argument('--rollout_length', '-r', type=int, default=2000)

    # for Swimmer-v1 and HalfCheetah-v1 use shift = 0
    # for Hopper-v1, Walker2d-v1, and Ant-v1 use shift = 1
    # for Humanoid-v1 used shift = 5
    # survival reward of Nabi
    parser.add_argument('--shift', type=float, default=3)
    parser.add_argument('--seed', type=int, default=237)
    parser.add_argument('--policy_type', type=str, default='linear')
    parser.add_argument('--dir_path', type=str, default='data')
    parser.add_argument('--redis-pswd', type=str, default="5241590000000000")
    ...
    ray.init(address=local_ip + ':6379', _redis_password=args.redis_pswd)
    params = vars(args)
    initNabi()
    run_ars(params)
